...
function IniciarBD(){
    ...
    request.onupgradeneeded=function (event){
        ...
        //Cogemos los datos de los archivos json
        var shops = new XMLHttpRequest();
        shops.onreadystatechange = function (){
            if (this.readyState == 4 && this.status == 200) {
                var shop =JSON.parse(this.responseText);
                //Recorrer las categorias del fichero json y las guardo en el almacen de categorias
                for (var i = 0; i < shop.shops.length; i++) {
                    objectStoreTiendas.add(shop.shops[i]);
                }
            }
        }
        shops.open("GET", "Shop.json", false);
        shops.send();
        ...
        categories.onreadystatechange = function (){
            if (this.readyState == 4 && this.status == 200) {
                ...
                //Recorrer las categorias del fichero json y las guardo en el almacen de categorias
                for (var i = 1; i < cat.categories.length; i++) {
                    objectStoreCategorias.add(cat.categories[i]);
                }
            }
        }
        categories.open("GET", "Category.json", false);
        categories.send();
        ...
        products.onreadystatechange = function (){
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (var i = 0; i < pro.products.length; i++) {
                    objectStoreProductos.add(pro.products[i]);
                }
            }
        }
    ...
    request.onsuccess=function(event){
        ...
        objectStoreRequestProducts.onsuccess = function(evt){
            ...
            var cat=Category.getObject(objectStoreRequestCategory.result[2]);
...
function cookForm(){
    ...
    if(cookie.length>0){ //si existe cookie
        ...
        var crearArchivo=document.getElementById("crearArchivo");
        crearArchivo.setAttribute("style", "display:block");
        crearArchivo.addEventListener("click", guardarDatos);
...
function guardarDatos(){
    ...
    var user="prueba";
    var cadeShop, cadeCategor, cadeProduct;
    ...
    request.onsuccess=function(event){
        ...
        //Datos del almacen Shop
        var transsho=db.transaction(["Shop"], "readwrite");
        var almsho=transsho.objectStore("Shop");

        almsho.openCursor().onsuccess=function(event){
            var cursor=event.target.result;

            if(cursor){
                var sho=cursor.value;
                shops.push(sho);

                cursor.continue();
            }else{
                cadeShop='{"shops": '+JSON.stringify(shops)+'}';

                //Datos del almacen Category
                var transcat=db.transaction(["Category"], "readwrite");
                var almcat=transcat.objectStore("Category");

                almcat.openCursor().onsuccess=function(event){
                    var cursor=event.target.result;
                    ...
                    if(cursor){
                        ...
                        cadeCategor='{"categories": '+JSON.stringify(categories)+'}';
        
                        //Datos del almacen products
                        var transpro=db.transaction(["Products"], "readwrite");
                        var almpro=transpro.objectStore("Products");

                        almpro.openCursor().onsuccess=function(event){
                            var cursor=event.target.result;
                
                            if(cursor){
                                var pro=cursor.value;
                                products.push(pro);
                
                                cursor.continue();
                            }else{
                                cadeProduct='{"products": '+JSON.stringify(products)+'}';
                
                                $.ajax({
                                    type: 'POST',
                                    url: 'js/guardar.php',
                                    data: {sh:cadeShop, cate:cadeCategor, prod:cadeProduct, user: user}
                                })
...
function insertarTienda(){
    ...
    if (cif == "") {
        ...
        var transShop = db.transaction(["Shop"],"readonly"); //abro transaccion de electura escritura sobre almacen Shop
        ...
        objectStoreShop.openCursor().onsuccess = function(e){
            ...
            if (result!==null) {
                if(result.value.cif === cif){ //si el cif coincide 
...
function insertarCat(){
    ...
    if (nombre == "") {
        ...
        var transCat = db.transaction(["Category"],"readonly"); //transaccion sobre almacen category
